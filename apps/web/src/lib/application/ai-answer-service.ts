  ...
  const deps = createDefaultAnswerPipelineDeps({
    ...
    onTrace: (trace: PipelineTrace) => {
      void emitRuntimeTrace({
        ...
        retrievalCounts: {
          internal: trace.internalCount ?? 0,
          web: trace.webCount ?? 0,
          research: trace.researchCount ?? 0,
          context: trace.contextCount ?? 0,
          citations: trace.citationCount ?? 0,
          sanitizedQueries: trace.sanitizedQueryCount ?? 0,
